...
src/
  ...
  responses.js      Response engine: keyword pools, typing phrases, getResponse routing (pure, DOM-free)
  histories.js      Pre-baked chat histories keyed by sidebar data-id (pure, DOM-free)
  avatar.js         Canonical Johnny Bravo SVG string (pure, DOM-free)
  state.js          App state: session token, currentChatId, isResponding, welcomeShown accessors
  ui.js             All chat UI: element lookups, rendering, event listeners (owns all chat DOM access)
  responses.test.js Vitest unit tests for the response engine, histories, avatar, and state
```

**Module boundary rule:** `responses.js`, `histories.js`, `avatar.js`, and `state.js` must stay DOM-free so tests can import them without a browser environment. All chat-surface DOM access lives in `ui.js`; canvas DOM access lives in `background.js`.

## Architecture

### 1. Canvas Background System (`src/background.js`)
...
- **Exports:** `initBackground()` wires resize handling and starts the loop; `spawnSparkles(x, y)` is called by `ui.js` on send.

### 2. Mock AI Response Engine (`src/responses.js`)
...
- **Tests:** `src/responses.test.js` covers category routing (including the greeting length guard), the default fallback, pool non-emptiness, history integrity, and the avatar markup.

### 3. State & Race Conditions (`src/state.js`)
...
- **`sessionToken`:** An incrementing integer, exposed via `currentSessionToken()` / `incrementSessionToken()`. Every time a new chat is started or a history item is loaded, the token increments (exactly in `loadChatHistory` and `startNewChat` in `ui.js`). Pending `setTimeout` callbacks check if their captured token matches the current token before firing. If they don't match, the callback is silently dropped.
- **`currentChatId`:** Tracks which chat history is currently being viewed (or `null` if it's a new live chat).

### 4. Pre-baked Chat Histories (`src/histories.js`)
...
- All styles live in `src/styles.css`, with colors defined via CSS variables in `:root`.
...
- It is injected into the welcome screen, AI message avatars, and the typing indicator by `ui.js`. Slightly simpler static variants live inline in `index.html` (sidebar footer and topbar).
